...
export default function EditRelationModal({
  ...
  useEffect(() => {
    if (open) {
      if (isCreate) {
        ...
        setSourceOptions(initialData.sourceName ? [{ value: `${initialData.sourceType}${ENTITY_SEP}${initialData.sourceName}`, label: `${initialData.sourceName} (${initialData.sourceType})` }] : [])
        setRelType(initialData.relationType)
        setTargetType(initialData.targetType)
        setTargetName(initialData.targetName)
        setTargetOptions(initialData.targetName ? [{ value: `${initialData.targetType}${ENTITY_SEP}${initialData.targetName}`, label: `${initialData.targetName} (${initialData.targetType})` }] : [])
      }
      // 打开时预取实体列表
      searchOntologyInstances(kbId, '', 20).then((res) => {
        const opts = buildEntityOptions(res.items || [])
        setSourceOptions((prev) => {
          // 保留已选中的项（创建模式 prev 为空，直接覆盖）
          if (isCreate) return opts
          const merged = [...prev]
          opts.forEach((o) => { if (!merged.some((m) => m.value === o.value)) merged.push(o) })
          return merged
        })
  ...
  const handleSave = () => {
    if (!sourceName.trim()) { message.warning(t('common.required')); return }
    if (!relType) { message.warning(t('common.required')); return }
    if (!targetName.trim()) { message.warning(t('common.required')); return }
    ...
    <Modal
      ...
      footer={
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12 }}>
          <Button size="middle" onClick={handleCancel}>{t('common.cancel')}</Button>
    ...
    >
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <Select
          ...
          onSearch={(kw) => { debouncedSourceSearch(kw) }}
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <Select
          ...
          onSearch={(kw) => { debouncedTargetSearch(kw) }}
