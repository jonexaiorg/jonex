...
export default function DomainKnowledgeInstanceList() {
  ...
  const breadcrumbItems = [
    ...
    { title: <span style={{ color: '#0b2b5c', fontWeight: 500 }}>{t('common.instance', { name: decodedType })}</span> },
  ];
    ...
    <div>
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: '#0b2b5c', margin: 0 }}>
            {decodedType} - {t('domainKnowledge.ontologyInstances')}
          </h2>
          <div style={{ fontSize: 13, color: '#94a3b8', marginTop: 4 }}>{t('domainKnowledge.instanceListDesc')}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 24, fontWeight: 700, color: '#3b82f6' }}>{total.toLocaleString()}</div>
          <div style={{ fontSize: 12, color: '#94a3b8' }}>{t('domainKnowledge.instanceTotalCount')}</div>
        </div>
      </div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          >
            <BlockOutlined style={{ color: '#3b82f6' }} /> {t('domainKnowledge.instanceList')}
          </h3>
          <Input
            prefix={<SearchOutlined />}
            placeholder={t('domainKnowledge.instanceSearchPlaceholder')}
            style={{ width: 200 }}
            value={keyword}
            onChange={(e) => {
          ...
          <>
            <Table
              columns={[
                ...
                {
                  title: t('domainKnowledge.sourceDocuments'),
                  key: 'docCount',
                  width: 80,
              ...
              size="middle"
              loading={loading}
              locale={{ emptyText: <span style={{ color: '#94a3b8' }}>{t('domainKnowledge.emptyInstances')}</span> }}
            />
