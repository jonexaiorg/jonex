...
export default function DomainKnowledgeDatasourceStorage() {
  ...
  const stats = [
    ...
    { label: t('dataSource.syncStatus'), value: ds?.lastSyncStatus || t('dataSource.notSynced'), color: '#f97316' },
  ...
  return (
    <div>
      ...
      <Spin spinning={loading}>
        ...
        >
          <div style={{ width: 48, height: 48, borderRadius: 12, background: '#fff7ed', color: '#f97316', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22, flexShrink: 0 }}>
            <FolderOpenOutlined />
          </div>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 4, fontSize: 13, color: '#64748b', flexWrap: 'wrap' }}>
              <span>{t('dataSource.storageDirect')}</span>
              <span>{cfg.backend || '-'}</span>
              <span>{t('dataSource.bucket')}：{cfg.bucket || '-'}{cfg.prefix ? ` / ${cfg.prefix}` : ''}</span>
        ...
        <Card style={{ borderRadius: 14, border: '1px solid #eef2f6', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }} bodyStyle={{ padding: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid #eef2f6' }}>
            ...
            <span style={{ fontSize: 12, color: '#94a3b8' }}>Documents are synchronized directly from storage; no manual upload is required.</span>
          </div>
