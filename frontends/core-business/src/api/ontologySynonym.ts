...
export async function listSynonyms(kbId: string, page = 1, pageSize = 20): Promise<SynonymListResult> {
...
export async function createSynonym(kbId: string, terms: string[], canonical?: string): Promise<SynonymGroup> {
...
export async function updateSynonym(synonymId: string, terms: string[], canonical?: string): Promise<SynonymGroup> {
...
export async function importSynonyms(kbId: string, groups: string[][]): Promise<SynonymImportResult> {
...
/** 循环分页拉全量，用于导出（一期不提供后端 export 接口）。 */
