        ...
        if (item.appId && item.internalPath) {
          ...
          if (item.matchPaths) {
            for (const mp of item.matchPaths) {
              const mpPath = `/apps/${mp.appId}/${mp.internalPath}`;
              if (pathname === mpPath || pathname.startsWith(mpPath + '/')) {
                return { section, leaf: item };
              }
            }
          }
...
export function getBreadcrumbItems(pathname: string): BreadcrumbEntry[] {
  ...
  if (result.leaf) {
    const leafPath =
      result.leaf.appId && result.leaf.internalPath ? `/apps/${result.leaf.appId}/${result.leaf.internalPath}` : '#';
