...
const getHostedInitialEntry = (basename: string) => {
  ...
  if (normalizedBasename) {
    if (path === normalizedBasename || path === `${normalizedBasename}/`) {
      ...
      // basename 不匹配时尝试已知前缀
      const knownPrefixes = ['/platform-management', '/apps/platform-management'];
...
const createAuthLoader =
  ...
  ({ request }: LoaderFunctionArgs) => {
    ...
    if (path === '/' || path === '/home') return redirect('/model-adapter');
...
const AppRoute = ({ basename, mode = 'standalone', shellContext }: AppRouteProps) => {
  const actualBasename = basename || (mode === 'hosted' ? '/apps/platform-management' : STANDALONE_BASENAME);
