...
class Parser:
    ...
    def parse_audio(
        ...
        **kwargs,
        ...
        """
        Parse audio file — minimal: just records the path. ASR is done by AsrModalProcessor.

        Supported formats: {AUDIO_FORMATS}

        Args:
            audio_path: Path to the audio file
            output_dir: Output directory path (unused, reserved for interface compatibility)
            **kwargs: Additional parameters (forwarded for interface compatibility)

        Returns:
            List[Dict[str, Any]]: Single-item content list with type "audio"
        """
...
class MineruParser(Parser):
    ...
    # Shared field aliases used by local, online, and self-hosted MinerU parsers.
    _FIELD_ALIASES = {
    ...
    def _run_mineru_command(
        ...
        # If not provided, falls back to MINERU_SOURCE env var, then huggingface
        ...
        **kwargs,
        ...
        Args:
            ...
            source: Model source (defaults to MINERU_SOURCE env var, then huggingface)
        ...
        # Source: explicit arg > env var > huggingface (MinerU default)
        if source is None:
    ...
    def _read_output_files(
        ...
        if json_file.exists():
            try:
                ...
                _FIELD_ALIASES = cls._FIELD_ALIASES
...
class MineruOnlineParser(MineruParser):
    ...
    def _upload_file(self, upload_url: str, file_path: Path) -> None:
        ...
        try:
            # urllib injects application/x-www-form-urlencoded when data is
            # present. MinerU signed OSS URLs may require an empty Content-Type,
            # so use http.client to send a raw PUT body.
...
class MineruSelfHostParser(MineruParser):
    """
    MinerU self-hosted API parser.

    Connects to the official self-hosted ``mineru-api`` (FastAPI) service
    instead of the mineru.net cloud API. Unlike the local ``mineru`` CLI, this
    parser is an HTTP-only client and does not require local models or a GPU.

    Supported API contract:
      - POST /tasks: upload a file and receive task/status/result identifiers
      - GET /tasks/{id}: poll pending -> processing -> completed/failed
      - GET /tasks/{id}/result: retrieve markdown, content list, and images

    Uses only the Python standard library and does not import the MinerU package.
    """
    ...
    def __init__(self) -> None:
        ...
        self.backend = os.environ.get(self.BACKEND_ENV, "pipeline").strip() or "pipeline"
        self.default_lang = os.environ.get(self.LANG_ENV, "ch").strip() or "ch"
        self.poll_interval = float(os.environ.get(self.POLL_INTERVAL_ENV, "5"))
        self.poll_timeout = int(os.environ.get(self.POLL_TIMEOUT_ENV, "1800"))
        self.return_images = (
            os.environ.get(self.RETURN_IMAGES_ENV, "false").strip().lower()
            in ("1", "true", "yes", "on")
        )
    ...
    def _post_multipart(
        self, path: str, file_path: Path, fields: Dict[str, Any], timeout: int = 300
    ) -> Dict[str, Any]:
        """构造 multipart/form-data 请求（仅用标准库）。

        fields 中的列表值会展开为重复字段（对应 FastAPI 的 List[...] Form）。
        """
        boundary = "----RAGAnythingBoundary" + hashlib.md5(os.urandom(16)).hexdigest()
        crlf = b"\r\n"
        body = bytearray()
        ...
        with open(file_path, "rb") as f:
            file_bytes = f.read()
        # 文件名 ASCII 安全化：中文/特殊字符在 multipart filename 头里会因服务端
        # （python-multipart）编码假设不同而损坏。MinerU 只按**扩展名**判类型，
        # 故把 stem 中的非 [A-Za-z0-9._-] 字符替换为 '_'，保留扩展名即可；单文件
        # 任务的结果按 results 首条兜底匹配（见 _fetch_result_item），不依赖文件名。
    ...
    def _ascii_safe_filename(name: str) -> str:
        """把文件名 ASCII 安全化，仅保留 [A-Za-z0-9._-]，其余替换为 '_'。

        保留扩展名供 MinerU 判类型；stem 为空/全非法时兜底为 'upload'。
        """
        safe = "".join(
    ...
    def _read_json_response(
        ...
        try:
            ...
            raise RuntimeError(
                f"MinerU self-host request failed: {label} HTTP {exc.code}: {detail}"
            ) from exc
        except urllib.error.URLError as exc:
            raise RuntimeError(
        ...
        try:
            ...
            raise RuntimeError(
                f"MinerU self-host returned non-JSON for {label}: {raw[:500]}"
            ) from exc

    # ── task lifecycle ──────────────────────────────────────────

    def _submit_task(
        self, file_path: Path, method: str, lang: Optional[str], **kwargs
    ) -> str:
        ...
        # lang_list / parse_method 仅 pipeline / hybrid 后端生效
        ...
        result = self._post_multipart("/tasks", file_path, fields, timeout=300)
        task_id = result.get("task_id")
        if not task_id:
            raise RuntimeError(f"MinerU self-host did not return task_id: {result}")
        self.logger.info(
    ...
    def _poll_task(self, task_id: str, timeout: Optional[int] = None) -> None:
        timeout = self.poll_timeout if timeout is None else int(timeout)
        deadline = time.monotonic() + timeout
        last_state = ""
        while time.monotonic() < deadline:
            ...
            if state != last_state:
                self.logger.info("MinerU self-host task %s: %s", task_id, state)
                last_state = state
            if state == "completed":
                return
            if state == "failed":
                raise RuntimeError(
                    f"MinerU self-host parsing failed: {status.get('error') or status}"
                )
            time.sleep(self.poll_interval)
        raise TimeoutError(
    ...
    def _fetch_result_item(
        ...
        payload = self._get_json(f"/tasks/{task_id}/result", timeout=120)
        results = payload.get("results") or {}
        if not results:
            raise RuntimeError(
                f"MinerU self-host result is empty for task {task_id}: {payload}"
            )
    ...
    def _build_content_list(
        ...
        if raw is None:
            ...
            return [{"type": "text", "text": md, "page_idx": 0}] if md else []
        # content_list 可能是 JSON 字符串（mineru-api 实测）或已是数组
        if isinstance(raw, str):
            try:
                content_list = json.loads(raw)
            except json.JSONDecodeError as exc:
                raise RuntimeError(
                    f"MinerU self-host content_list is not valid JSON: {raw[:500]}"
                ) from exc
        else:
            content_list = raw
        if not isinstance(content_list, list):
            raise RuntimeError(
                f"MinerU self-host content_list has unexpected type: {type(content_list)}"
            )

        # 落盘图片（若服务返回 images）并重写路径
        ...
        _FIELD_ALIASES = self._FIELD_ALIASES
        resolved_base = images_base_dir.resolve()
        for item in content_list:
            if not isinstance(item, dict):
                continue
            ...
            for field_name in ["img_path", "table_img_path", "equation_img_path"]:
                ...
                if not self.return_images:
                    # 未拉取图片：清空本地路径，下游按无图降级
                    item[field_name] = ""
                    continue
                candidate = (images_base_dir / Path(str(item[field_name])).name).resolve()
                if not candidate.is_relative_to(resolved_base) or not candidate.exists():
                    item[field_name] = ""
                    continue
    ...
    def _dump_images(images: Dict[str, Any], out_dir: Path) -> None:
        """把响应内的图片（base64 / dataurl）落盘到 out_dir，文件名取 key 的 basename。"""
    ...
    def _parse_via_selfhost(
        ...
        **kwargs,
        ...
        if output_dir:
            ...
            base_output_dir = file_path.parent / "mineru_selfhost_output"
        base_output_dir.mkdir(parents=True, exist_ok=True)
    ...
    def parse_pdf(
        ...
        **kwargs,
        ...
        return self._parse_via_selfhost(
            pdf_path, output_dir=output_dir, method=method, lang=lang, **kwargs
        )
    ...
    def parse_document(
        ...
        **kwargs,
        ...
        # mineru-api 统一吃各类文件，无需按扩展名分流
        return self._parse_via_selfhost(
            file_path, output_dir=output_dir, method=method, lang=lang, **kwargs
        )

    def check_installation(self) -> bool:
        # 仅需内网服务可达；探活失败不阻断（由实际解析时报错）。
        try:
            health = self._get_json("/health", timeout=10)
            if health.get("status") == "healthy":
                return True
            self.logger.warning(
                "MinerU self-host /health status=%s", health.get("status")
            )
        except Exception as exc:  # noqa: BLE001
            self.logger.warning(
                "MinerU self-host health check failed (%s): %s", self.base_url, exc
            )
        return True
...
def main():
    ...
    parser.add_argument(
        ...
        default="mineru",
        help=(
            "Parser selection. Built-ins: mineru, mineru_online, mineru_selfhost, docling, paddleocr. "
