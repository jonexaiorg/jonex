...
>
> **📦 离线部署**: 对于离线或隔离环境，请参阅[离线部署指南](https://github.com/HKUDS/LightRAG/blob/main/docs/OfflineDeployment.md)，了解预安装所有依赖项和缓存文件的说明。
...
# 已发布的 Server wheel 已包含预构建 WebUI。
# 配置 .env 后即可启动服务。
...
直接执行 `uv build` 只会打包当前已有文件，不会自动编译前端。发布可运行 WebUI 的 Server wheel 时，请使用跨平台发布脚本：

```bash
# 需要 PATH 中已有 bun 和 uv。脚本会以 frozen 模式使用 bun.lock，
# 构建 WebUI 和 wheel，并校验 wheel 中存在 lightrag/api/webui/index.html。
python scripts/build_server_wheel.py
```

生成的 wheel 位于 `dist/`。Docker 构建已通过 frontend-builder 阶段完成同等流程，无需调用该脚本。未执行此步骤而直接构建的 wheel 只能视为 core/API-only 产物，不应作为包含 WebUI 的 Server wheel 发布。
...
>
> 由 GitHub Actions 发布到 GHCR 的官方镜像已使用 GitHub OIDC 和 Sigstore Cosign 进行签名。校验方式请参阅 [docs/DockerDeployment.md](https://github.com/HKUDS/LightRAG/blob/main/docs/DockerDeployment.md#verify-official-ghcr-images-with-cosign)。
...
每个目标的详细说明请参阅 [docs/InteractiveSetup.md](https://github.com/HKUDS/LightRAG/blob/main/docs/InteractiveSetup.md)。
...
LightRAG 服务器旨在提供 Web UI 和 API 支持，同时提供了全面的知识图谱可视化功能，支持各种重力布局、节点查询、子图过滤等。有关LightRAG服务器的更多信息，请参阅[LightRAG服务器](https://github.com/HKUDS/LightRAG/blob/main/docs/LightRAG-API-Server-zh.md)。
...
完整的 Core API 参考 —— 包括初始化参数、`QueryParam`、各 LLM/Embedding 接入示例（OpenAI、Ollama、Azure、Gemini、HuggingFace、LlamaIndex）、Rerank 注入、插入操作、实体/关系管理、删除与合并 —— 详见 **[docs/ProgramingWithCore.md](https://github.com/HKUDS/LightRAG/blob/main/docs/ProgramingWithCore.md)**（英文）。

> ⚠️ **如果您希望将LightRAG集成到您的项目中，建议您使用LightRAG Server提供的REST API**。LightRAG Core通常用于嵌入式应用，或供希望进行研究与评估的学者使用。

### 高级功能

LightRAG 提供 Token 用量追踪、知识图谱数据导出、LLM 缓存管理、Langfuse 可观测性集成和基于 RAGAS 的评估框架。详见 **[docs/AdvancedFeatures.md](https://github.com/HKUDS/LightRAG/blob/main/docs/AdvancedFeatures.md)**（英文）。

### 多模态文档处理（RAG-Anything 集成）

LightRAG 与 [RAG-Anything](https://github.com/HKUDS/RAG-Anything) 集成，支持对 PDF、Office 文档、图像、表格和公式的端到端多模态 RAG。详见 **[docs/AdvancedFeatures.md](https://github.com/HKUDS/LightRAG/blob/main/docs/AdvancedFeatures.md)**（英文）。

> LightRAG Server 将会在不久的将来把 RAG-Anything 的多模态处理能力整合到其文件件处理流水线中。敬请期待。

## 重现论文结果

LightRAG 在农业、计算机科学、法律和混合等领域均显著优于 NaiveRAG、RQ-RAG、HyDE 和 GraphRAG。完整评估方法论、提示词和复现步骤详见 **[docs/Reproduce.md](https://github.com/HKUDS/LightRAG/blob/main/docs/Reproduce.md)**（英文）。
