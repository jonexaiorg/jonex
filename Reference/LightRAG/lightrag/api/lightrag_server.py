...
from lightrag.api.workspace_manager import (
    WorkspaceRAGManager,
    get_workspace_from_request,  # [jonex] extracted from create_application()
)
...
def create_app(args):
    ...
    async def lifespan(app: FastAPI):
        ...
        try:
            # [jonex] Initialize default workspace instance via manager.
            ...
            # [jonex] Clean up all workspace instances through manager
    ...
    # ── [jonex] 计量上下文中间件 ───────────────────────────────────────
    # 从入站 X-Jonex-* 头提取 tenant/kb/scene/trace 存入 contextvar，
    ...
    # ── [jonex] Build factory for WorkspaceRAGManager ─────────────────
    ...
    # ── [jonex] WebUI workspace 切换器（调试用，默认关闭）────────────────
