...
class Neo4JStorage(BaseGraphStorage):
    ...
    # ── [jonex] 穷举分页 / 计数 / 分布 ──────────────────────────────
    # 供 atomic-rag 的 HTTP reader（LightRAGGraphReader）穷举某 workspace（租户+知识库）
    # 的全部实体/关系，替代其原先直接读本地 vdb_*.json。所有方法均按 workspace 标签隔离，
    # file_path/doc_id 过滤口径与平台 file_source 对齐：
    #   - doc_id：file_source 内含 `doc=<document_id>|`，用 CONTAINS 精确到 `|` 边界；
    #   - file_path：合并实体的 file_path 为多个 file_source 以 <SEP> 连接，用 split+IN 精确成员相等，
    #     禁止裸 CONTAINS（避免 file_a.txt 误命中 file_a.txt.bak）。
    # 归档见 Reference/LightRAG/JONEX_CHANGES.md。

    @staticmethod
    def _jonex_scope_conditions(alias: str, doc_id: Optional[str], file_path: Optional[str]) -> list[str]:
        conds: list[str] = []
        if doc_id:
            conds.append(f"{alias}.file_path CONTAINS ('doc=' + $doc_id + '|')")
        if file_path:
            conds.append(f"$file_path IN split(coalesce({alias}.file_path,''), '<SEP>')")
        return conds

    @READ_RETRY
    async def jonex_get_entities_page(
        self,
        *,
        skip: int = 0,
        limit: int = 50,
        ...
        """穷举分页返回实体（workspace 隔离 + 可选 doc/file/keyword/type 过滤 + 可选度数）。"""
        workspace_label = self._get_workspace_label()
        conds = ["n.entity_id IS NOT NULL"]
        conds += self._jonex_scope_conditions("n", doc_id, file_path)
        ...
        if entity_type:
            conds.append("n.entity_type = $entity_type")
            params["entity_type"] = entity_type
        if keyword:
            conds.append(
                "(toLower(coalesce(n.entity_id,'')) CONTAINS toLower($kw) "
                "OR toLower(coalesce(n.description,'')) CONTAINS toLower($kw))"
            )
            params["kw"] = keyword
        where = "WHERE " + " AND ".join(conds)
        if with_degree:
            degree_block = "OPTIONAL MATCH (n)-[r]-()\n            WITH n, count(r) AS degree"
        else:
            degree_block = "WITH n, 0 AS degree"
        query = f"""
            MATCH (n:`{workspace_label}`)
            {where}
            {degree_block}
            RETURN n.entity_id AS entity_name, n.entity_type AS entity_type,
                   ...
                   n.file_path AS file_path, n.created_at AS created_at, degree
            ORDER BY entity_name SKIP $skip LIMIT $limit
        """
        async with self._driver.session(
            database=self._DATABASE, default_access_mode="READ"
        ) as session:
    ...
    @READ_RETRY
    async def jonex_get_relations_page(
        self,
        *,
        skip: int = 0,
        limit: int = 50,
        ...
        """穷举分页返回关系（有向去重 a->b，workspace 隔离 + 可选过滤）。"""
        workspace_label = self._get_workspace_label()
        conds = self._jonex_scope_conditions("r", doc_id, file_path)
        params: dict[str, Any] = {"skip": int(skip), "limit": int(limit)}
        if doc_id:
            params["doc_id"] = doc_id
        if file_path:
            params["file_path"] = file_path
        if source_entity:
            conds.append("a.entity_id = $source_entity")
            params["source_entity"] = source_entity
        if target_entity:
            conds.append("b.entity_id = $target_entity")
            params["target_entity"] = target_entity
        if keyword:
            conds.append(
                "(toLower(coalesce(a.entity_id,'')) CONTAINS toLower($kw) "
                "OR toLower(coalesce(b.entity_id,'')) CONTAINS toLower($kw) "
                "OR toLower(coalesce(r.description,'')) CONTAINS toLower($kw))"
            )
            params["kw"] = keyword
        ...
        query = f"""
            MATCH (a:`{workspace_label}`)-[r]->(b:`{workspace_label}`)
            {where}
            RETURN a.entity_id AS src_id, b.entity_id AS tgt_id,
                   r.description AS description, r.keywords AS keywords,
                   r.weight AS weight, r.source_id AS source_id,
                   r.file_path AS file_path, r.created_at AS created_at
            ORDER BY src_id, tgt_id SKIP $skip LIMIT $limit
        """
        async with self._driver.session(
            database=self._DATABASE, default_access_mode="READ"
        ) as session:
    ...
    @READ_RETRY
    async def jonex_get_graph_counts(
        self,
        *,
        doc_id: Optional[str] = None,
        file_path: Optional[str] = None,
        ...
        """实体/关系总数（workspace 隔离 + 可选过滤），供 get_summary/分页 total。

        source_entity/target_entity 仅作用于关系计数（与 jonex_get_relations_page 一致，
        保证带端点过滤的关系分页 total 正确）；实体计数不受其影响。
        """
        workspace_label = self._get_workspace_label()
        n_conds = ["n.entity_id IS NOT NULL"] + self._jonex_scope_conditions("n", doc_id, file_path)
        r_conds = self._jonex_scope_conditions("r", doc_id, file_path)
        params: dict[str, Any] = {}
        if doc_id:
            params["doc_id"] = doc_id
        if file_path:
            params["file_path"] = file_path
        if source_entity:
            r_conds.append("a.entity_id = $source_entity")
            params["source_entity"] = source_entity
        if target_entity:
            r_conds.append("b.entity_id = $target_entity")
            params["target_entity"] = target_entity
        n_where = "WHERE " + " AND ".join(n_conds)
        r_where = ("WHERE " + " AND ".join(r_conds)) if r_conds else ""
        async with self._driver.session(
            database=self._DATABASE, default_access_mode="READ"
        ) as session:
            r1 = await session.run(
                f"MATCH (n:`{workspace_label}`) {n_where} RETURN count(n) AS c", **params
            )
            entities_count = (await r1.single())["c"]
            await r1.consume()
            r2 = await session.run(
                f"MATCH (a:`{workspace_label}`)-[r]->(b:`{workspace_label}`) {r_where} "
                "RETURN count(r) AS c",
                **params,
            )
            relationships_count = (await r2.single())["c"]
            await r2.consume()
            return {
                "entities_count": int(entities_count or 0),
                "relationships_count": int(relationships_count or 0),
            }

    @READ_RETRY
    async def jonex_get_graph_distribution(
        self, *, doc_id: Optional[str] = None, file_path: Optional[str] = None
    ) -> dict:
        """实体类型分布 + 总节点/边数（服务端聚合，供 get_graph_summary）。"""
        workspace_label = self._get_workspace_label()
        n_conds = ["n.entity_id IS NOT NULL"] + self._jonex_scope_conditions("n", doc_id, file_path)
        r_conds = self._jonex_scope_conditions("r", doc_id, file_path)
        params: dict[str, Any] = {}
        ...
        async with self._driver.session(
            ...
            r1 = await session.run(
                f"MATCH (n:`{workspace_label}`) {n_where} "
                "RETURN coalesce(n.entity_type,'unknown') AS t, count(n) AS c "
                "ORDER BY c DESC",
                **params,
            )
            ...
            async for record in r1:
                entity_type_distribution[record["t"]] = int(record["c"])
                total_nodes += int(record["c"])
            await r1.consume()
            r2 = await session.run(
                f"MATCH (a:`{workspace_label}`)-[r]->(b:`{workspace_label}`) {r_where} "
                "RETURN count(r) AS c",
            ...
            total_edges = int((await r2.single())["c"] or 0)
            await r2.consume()
            return {
