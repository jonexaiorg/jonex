...
class DomainRAGText(DomainCapability):
    ...
    async def validate_input(self, request: CapabilityRequest) -> bool:
        if not request.payload:
            raise InvalidParameterError(message=translate("err.rag.payload_required", fallback="文本 RAG 请求 payload 不能为空"))
        ...
        if action == "ingest" and "file_path" not in request.payload:
            raise InvalidParameterError(message=translate("err.capability.missing_action_param", params={"action": "ingest", "param": "file_path"}, fallback="ingest 必须提供 file_path 参数"))
        if action == "query" and "query" not in request.payload:
            raise InvalidParameterError(message=translate("err.capability.missing_action_param", params={"action": "query", "param": "query"}, fallback="query 必须提供 query 参数"))
        if action == "delete" and "doc_id" not in request.payload:
            raise InvalidParameterError(message=translate("err.capability.missing_action_param", params={"action": "delete", "param": "doc_id"}, fallback="delete 必须提供 doc_id 参数"))
        if action == "get_task_status" and "task_id" not in request.payload:
            raise InvalidParameterError(message=translate("err.capability.missing_action_param", params={"action": "get_task_status", "param": "task_id"}, fallback="get_task_status 必须提供 task_id 参数"))
        return True
